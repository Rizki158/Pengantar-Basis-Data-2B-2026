...
WHERE EXISTS
(SELECT nama FROM mahasiswa WHERE nama = 'Budi Santoso');

SELECT nim,nama 
FROM mahasiswa WHERE EXISTS
(SELECT nilai_angka FROM nilai); 

SELECT AVG(nilai_angka)AS Rata_rata_nilai
FROM (SELECT nilai_angka FROM nilai WHERE kode_mk IN ('MK01','MK02'))AS Data_nilai;

CREATE VIEW v_transkip_lengkap AS 
SELECT m.nim,m.nama AS nama_mahasiswa, mk.nama_mk, n.nilai_huruf FROM mahasiswa m
JOIN nilai n ON m.nim = n.nim
JOIN mata_kuliah mk ON n.kode_mk = mk.kode_mk;

SELECT * 
FROM v_transkip_lengkap
WHERE nilai_huruf ='A';